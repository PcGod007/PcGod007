...
USER_NAME = os.environ.get('USER_NAME', 'PcGod007')
...
def stars_counter(data):
    ...
    total_stars = 0
    for node in data:
        if node['node'] is not None:  # skip deleted/inaccessible repos
...
if __name__ == '__main__':
    ...
    """
    if not access_token:
        print("Warning: ACCESS_TOKEN not found in environment. Skipping dynamic API statistics updates.")
        # Local mock run to ensure files format successfully and check date calculation.
        # Birthday: 03 July 2003 — uptime is Pranav's age
        birthday = datetime.datetime(2003, 7, 3)
        ...
        contrib_data = "3"
        follower_data = "0"
        total_loc = [3142500, 1064100, 2078400, True]
        
        svg_overwrite('dark_mode.svg', age_data, commit_data, star_data, repo_data, contrib_data, follower_data, total_loc[:-1])
        svg_overwrite('light_mode.svg', age_data, commit_data, star_data, repo_data, contrib_data, follower_data, total_loc[:-1])
        print(f"Successfully ran local mock update. Uptime set to: {age_data}")
        exit(0)

    print('Calculation times:')
    # define global variable for owner ID and calculate user's creation date
    # e.g {'id': 'MDQ6VXNlcjU3MzMxMTM0'} and 2019-11-03T21:15:07Z for username 'Andrew6rant'
    # Birthday: 03 July 2003 — uptime = Pranav's real age
    birthday = datetime.datetime(2003, 7, 3)
